...
import           Cardano.Plutus.Nft                  as ExplorieNft
import           Plutus.Trace.Emulator.Extract       (writeScriptsTo, ScriptsConfig (..), Command (..))
import           Ledger.Index                        (ValidatorMode(..))

main :: IO ()
main = void $ Simulator.runSimulationWith handlers $ do
    Simulator.logString @(Builtin StarterContracts) "Starting PAB webserver on port 8080. Press enter to exit."
...
writeCostingScripts = do
  ...
  (totalSize, exBudget) <- writeScriptsTo config "explorie" trace def
...
instance Builtin.HasDefinitions StarterContracts where
    getDefinitions = [ExplorieContract]
    getSchema =  \case
        ExplorieContract -> Builtin.endpointsToSchemas @ExplorieNft.ExplorieSchema
    getContract = \case
        ExplorieContract -> SomeBuiltin (ExplorieNft.game @ContractError)
